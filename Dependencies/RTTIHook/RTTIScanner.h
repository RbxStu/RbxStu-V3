...
public:
    struct SectionData {
        SectionData(PEParser::PESections *text, PEParser::PESections *data,
                    PEParser::PESections *rdata) : text(text), data(data), rdata(rdata) {
        }
...
#pragma pack(pop)
    ...
    public:
        RTTI(void **pVFT,
             ...
             RTTIScanner::BaseClassDescriptor *pBCD) : pVirtualFunctionTable(pVFT),
        ...
        static std::string demangleName(const char *name) {
            ...
            if (!UnDecorateSymbolName(name, output, sizeof(output),
                                      UNDNAME_NO_ARGUMENTS | UNDNAME_NAME_ONLY | UNDNAME_32_BIT_DECODE |
                                      UNDNAME_NO_MS_KEYWORDS | UNDNAME_NO_LEADING_UNDERSCORES)) {
                return "";
            } else {
                return output;
            }
    ...
    ~RTTIScanner() {
        RTTIScanner::parser.reset();
        RTTIScanner::classRTTI.clear();
    ...
    bool scan(PEParser::ProcessInfo *pInfo = nullptr) {
        ...
        for (auto &section: *rdata) {
            ...
            while (pCOL++ < end) {
                ...
                RTTIScanner::classRTTI.emplace(
                    name, std::make_shared<RTTI>(reinterpret_cast<void **>(pCOL), COL, TD, CHD, pBCD));
            }
        }
    ...
    static inline std::unordered_map<std::string_view, std::shared_ptr<RTTI> > classRTTI;
...
private:
    ...
    // REX.W lea reg1,[rip]
    // REX.W mov [reg2],reg1
    const __m128i signature = _mm_setr_epi8(0x48, 0x8D, 0x05, 0x0, 0x0, 0x0, 0x0, 0x48, 0x89, 0x0, 0x0, 0x0, 0x0, 0x0,
                                            0x0, 0x0);
    const __m128i bitmask = _mm_setr_epi8(0b00000100, 0, 0b00111000, -1, -1, -1, -1, 0b00000101, 0, 0b00111111, -1, -1,
                                          -1, -1, -1, -1);

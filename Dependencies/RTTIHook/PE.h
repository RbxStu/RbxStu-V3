...
public:
    ...
    public:
        ...
        template<typename T>
        ibo32(T *address) {
            this->val = static_cast<int>(reinterpret_cast<uintptr_t>(address) - reinterpret_cast<uintptr_t>(this->
                                             getProcessInfo()->mInfo->lpBaseOfDll));
        }
        ...
        ibo32(T1 *address, T2 *base) noexcept {
        ...
        T as() {
            return reinterpret_cast<T>(reinterpret_cast<unsigned char *>(this->getProcessInfo()->mInfo->lpBaseOfDll) +
                                       this->val);
        }
        ...
        /// <returns>a pointer calculated from the ibo</returns>
        template<typename T1 = unsigned char *, typename T2>
        T1 as(T2 *base) noexcept {
    ...
    bool parse(ProcessInfo *pInfo = nullptr) {
        ...
        for (int i = 0; i < sectionCount; i++) {
            ...
            section->end = static_cast<int>(section->start.as() + section->size);
    ...
    template<typename T>
    static bool isAddressInSection(T *address, PESections *sections) {
    ...
    /// <returns>true if address is in one of the sections, otherwise false</returns>
    template<typename T>
    bool isAddressInSection(T *address, std::string name) {
